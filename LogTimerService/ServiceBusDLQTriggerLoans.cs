...
{
    public class ServiceBusDLQTriggerLoans
    {
        [FunctionName("ServiceBusDLQTriggerLoans")]
        public async Task RunAsync([ServiceBusTrigger("ms-loans/$DeadLetterQueue", Connection = "rasputinServicebus")]Microsoft.Azure.ServiceBus.Message myQueueItem, ILogger log)
        {
            var body = Encoding.UTF8.GetString(myQueueItem.Body);
            log.LogInformation($"ms-loans/$DeadLetterQueue: {body}");

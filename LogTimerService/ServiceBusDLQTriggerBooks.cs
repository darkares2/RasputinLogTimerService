...
{
    public class ServiceBusDLQTriggerBooks
    {
        [FunctionName("ServiceBusDLQTriggerBooks")]
        public async Task RunAsync([ServiceBusTrigger("ms-books/$DeadLetterQueue", Connection = "rasputinServicebus")]Microsoft.Azure.ServiceBus.Message myQueueItem, ILogger log)
        {
            var body = Encoding.UTF8.GetString(myQueueItem.Body);
            log.LogInformation($"ms-books/$DeadLetterQueue: {body}");

...
{
    public class ServiceBusDLQTriggerUsers
    {
        [FunctionName("ServiceBusDLQTriggerUsers")]
        public async Task RunAsync([ServiceBusTrigger("ms-users/$DeadLetterQueue", Connection = "rasputinServicebus")]Microsoft.Azure.ServiceBus.Message myQueueItem, ILogger log)
        {
            var body = Encoding.UTF8.GetString(myQueueItem.Body);
            log.LogInformation($"ms-users/$DeadLetterQueue: {body}");
